...
import {parseSemanticBranchName} from './semantic-release-merge'
...
export function init(): IInitReturnValue | undefined {
  ...
  const targetBranch = getPRTargetBranchName(pushDescription)
  
  // Check if this is a semantic version branch
  const semanticVersion = parseSemanticBranchName(targetBranch)
  
  if (!semanticVersion) {
    debug(`Not a semantic version branch: ${targetBranch}. This action only supports semantic version branches (e.g., 1.x, 1.1.x)`)
    return
  }
  
  debug(`Semantic version branch detected: ${targetBranch}`, semanticVersion)
